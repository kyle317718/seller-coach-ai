...
// 실전 액션 답변 생성 엔드포인트
app.post('/api/action-answer', async (req, res) => {
  try {
    const { productInfo, step, checklistKey } = req.body;
    if (!DEEPSEEK_API_KEY) {
      return res.status(500).json({ answer: 'DEEPSEEK API KEY가 설정되지 않았습니다.' });
    }
    // 프롬프트 생성
    const prompt = `상품명: ${productInfo?.name || productInfo?.title || ''}\n상품 설명: ${productInfo?.description || ''}\n분석 단계: ${step}\n실전 가이드 항목: ${checklistKey}\n\n위 상품과 항목에 대해, 셀러가 바로 따라할 수 있는 구체적이고 실용적인 실전 답변(예시, 액션 플랜, 체크리스트 등)을 3~5줄로 제시해 주세요.\n- 반드시 실전 중심, 바로 실행 가능한 방식으로 작성\n- 마크다운/특수문자 없이 일반 텍스트로만 응답`;
    const response = await axios.post(
      DEEPSEEK_API_URL,
      {
        model: 'deepseek-chat',
        messages: [
          {
            role: 'system',
            content: '당신은 전문적인 이커머스 셀러 코치입니다. 상품의 시장성과 판매 전략, 실전 실행법을 분석하여 실용적이고 구체적인 조언을 제공합니다. 반드시 실전 중심, 바로 실행 가능한 방식으로 답변하세요.'
          },
          {
            role: 'user',
            content: prompt
    ...
    const answer = removeMarkdownSymbols(response.data.choices[0].message.content);
    res.json({ answer });
  } catch (e) {
    console.error('실전 답변 생성 오류:', e);
    res.status(500).json({ answer: 'AI 실전 답변 생성 중 오류가 발생했습니다.' });
  }
});
